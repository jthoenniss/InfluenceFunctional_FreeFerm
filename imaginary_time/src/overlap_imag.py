...
def construct_grassmann_exponential(B: np.ndarray, E_up: float, E_down: float, t:float, delta_tau:float) -> np.ndarray:
    """
    ...
    Important: the matrix B must be entered in the convention where the Grassmann variables are arranged in DECREASING order, 
    i.e. first the late times, then the early time. 
    The last varaible is the ingoing leg at time 0, the first variable is the outgoing leg the the final time
...
def compute_propagator_grassmann(exponent: np.ndarray) -> np.ndarray:
    """
